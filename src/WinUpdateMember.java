...
public class WinUpdateMember extends JDialog {
	...
	public static void main(String[] args) {
		try {
			WinUpdateMember dialog = new WinUpdateMember("1111111234567");
	 ...
	 * Create the dialog.
	 */
	public WinUpdateMember(String Jumin) {
		setTitle("동문 주소록");
		...
		JButton btnInsertMember = new JButton("회원 변경");
	...
	private void init(String jumin) {
		try {
			...
			String sql = "select * from addressTBL where jumin='";
			sql = sql + jumin + "'";
			...
			if(rs.next()) {
				...
				tfJumin2.setText(jumin.substring(6));
		...
		try {
			...
			Connection con = 
					DriverManager.getConnection(
							...
							"12345");
			Statement stmt = con.createStatement();			
			String sql = "update addressTBL set name='";
			sql = sql + tfName.getText() + "', mobile='";
			sql = sql + tfMobile.getText() + "', address='" ;
			sql = sql + tfAddress.getText() + "', company='" ;
			sql = sql + tfCompany.getText() + "', pic='";
			sql = sql + picPath.replaceAll("\\\\", "/") + "'";
			sql = sql + " where jumin='" + tfJumin1.getText();
			sql = sql + String.valueOf(tfJumin2.getPassword()) + "'";
